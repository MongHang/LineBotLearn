...
def getCurrencyName(currency):
    ...
    # currency_list.get(currency, default=None)
    try:
        currency_name = currency_list[currency]
    except:
        # return "無可支援的外幣"
        return False, "無可支援的外幣"
    # return currency_name
    return True, currency_name

    # if currency in currency_list.keys():
...
def showCurrency(code) -> "JPY":
    ...
    currency_name = getCurrencyName(code)[1]
    if currency_name == "無可支援外幣":
        return "無可支援外幣"
    ...
    sold_cash = "無資料" if currency[1] == "-" else str(float(currency[1]))

    buying_spot = "無資料" if currency[1] == "-" else str(float(currency[1]))

    sold_spot = "無資料" if currency[1] == "-" else str(float(currency[1]))

    # content +=  f"{currency_name} 最新掛牌時間為: {now_time}\n ---------- \n 現金買入價格: {buying_cash}\n 現金賣出價格: {sold_cash}\n 即期買入價格: {buying_spot}\n 即期賣出價格: {sold_spot}\n \n"

    content += f'''
{currency_name} 最新掛牌時間為: {now_time}
 ---------- 
現金買入價格: {buying_cash}
現金賣出價格: {sold_cash}
即期買入價格: {buying_spot}
